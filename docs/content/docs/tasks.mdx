...
3. Select a [provider](/docs/providers) (Claude Code, Codex, Qwen, etc.)
4. Click **Create**
...
For additional isolation, tasks can run in [containers](/docs/containerization).

## Multi-Agent Mode

Run [multiple agents in parallel](/docs/parallel-agents) on the same problem:

- Select multiple providers (e.g., Claude Code + Codex)
- Or run the same provider multiple times (e.g., 3× Claude Code)

Each agent gets its own worktree. Compare approaches and pick the best result. See [Best of N](/docs/best-of-n).
...
- **Link an issue**: Attach a Linear, Jira, or GitHub issue (see [Passing Issues](/docs/issues))

...
After agents finish, review their changes in the [diff view](/docs/diff-view). Each agent's worktree has its own set of changes. Check the diff stats (files changed, lines added/removed) for a quick sense of each approach.

...
const SPECIAL_FILES: Record<string, { icon: React.ElementType; color: string; label: string }> = {
  ...
  '.env.local': { icon: VscSettingsGear, color: 'text-yellow-600', label: 'Local environment file' },
  '.env.production': { icon: VscSettingsGear, color: 'text-yellow-600', label: 'Production environment file' },
  '.env.development': { icon: VscSettingsGear, color: 'text-yellow-600', label: 'Development environment file' },
  ...
  'next.config.js': { icon: SiNextdotjs, color: 'text-gray-800 dark:text-white', label: 'Next.js config' },
...
> {
  ...
  render(): ReactNode {
    if (this.state.hasError) {
      return this.props.fallback || <VscError className="h-4 w-4 text-red-500" aria-label="Icon load error" />;
    }
...
export const FileIcon = memo<FileIconProps>(({
  ...
  if (isLoading) {
    return (
      <div
        className={`${className} animate-pulse bg-gray-300 dark:bg-gray-600 rounded`}
  ...
  return (
    <IconErrorBoundary fallback={<VscFile {...iconProps} className={`${className} text-gray-500`} aria-label={ariaLabel} />}>
      <IconComponent
        {...iconProps}

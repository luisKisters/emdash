...
export class TabManagerStore implements Snapshottable<TabManagerSnapshot> {
  ...
  get openFilePaths(): string[] {
    return this.tabs
      .filter((t): t is FileTabState => t.kind === 'file' && !t.isExternal)
      .map((t) => t.path);
  }
  ...
  get resolvedTabs(): ResolvedTab[] {
    return this.tabs
      .map((tab): ResolvedTab | null => {
        ...
        const bufferUri = tab.isExternal ? '' : buildMonacoModelPath(this.modelRootPath, tab.path);
        return {
          kind: 'file',
          tabId: tab.tabId,
          path: tab.path,
          isPreview: tab.isPreview,
          isDirty: tab.isExternal ? false : modelRegistry.dirtyUris.has(bufferUri),
          bufferUri,
          isActive: this.activeTabId === tab.tabId,
        };
      })
  ...
  restoreSnapshot(snapshot: Partial<TabManagerSnapshot>): void {
    if (snapshot.tabs) {
      this.tabs = snapshot.tabs.map((t): TabState => {
        ...
        const tab = this._makeFileTab(t.path, t.isPreview, t.tabId);
        if (t.isExternal) {
          tab.isExternal = true;
          tab.isLoading = true;

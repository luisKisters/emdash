...
import { sshConnections as sshConnectionsTable, type SshConnectionInsert } from '../_new/db/schema';
...
export const sshController = createRPCController({
      ...
      return new Promise(async (resolve) => {
        ...
        if (config.authType === 'password') {
          ...
          try {
            ...
            resolve({
              success: false,
              error: `Failed to read private key: ${(err as Error).message}`,
              debugLogs,
            });
            return;
          }
  ...
  saveConnection: async (
    ...
    try {
      ...
      void import('../_new/telemetry').then(({ capture }) => {
  ...
  deleteConnection: async (id: string): Promise<{ success: boolean; error?: string }> => {
    try {
  ...
  connect: async (
    ...
    try {
      ...
      if (config.authType === 'key' && typeof config.passphrase === 'string' && config.passphrase) {
  ...
  disconnect: async (connectionId: string): Promise<{ success: boolean; error?: string }> => {
    try {
      // Stop monitoring BEFORE disconnecting so the monitor's
      // handleDisconnect listener doesn't trigger a reconnect
...
/**
 * Register SSH side effects (monitor reconnect handler).
 * All request/response IPC is handled via sshController in the RPC router.
 */
export function registerSshIpc() {
  // Wire up reconnect handler so the monitor's reconnect event actually reconnects

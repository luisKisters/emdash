...
import { projectSettingsService } from '../core/projects/settings/ProjectSettingsService';
import { createLocalWorktreeService } from '../core/worktrees/_WorktreeService';
...
export const projectSettingsController = createRPCController({
  ...
  fetchBaseRef: async (
    ...
    const settings = await projectSettingsService.getProjectSettings(projectId);
    const storedBaseRef = settings?.baseRef ?? 'HEAD';

    const worktreesDir = path.join(projectPath, '..', 'worktrees');
    const svc = createLocalWorktreeService(projectPath, worktreesDir);
    const fullRef = await svc.fetchLatestBaseRef(storedBaseRef);

    // Parse remote/branch from a ref like "origin/main" or just "main"
    const slashIdx = fullRef.indexOf('/');
    const remote = slashIdx > 0 ? fullRef.slice(0, slashIdx) : '';
    const branch = slashIdx > 0 ? fullRef.slice(slashIdx + 1) : fullRef;

    return { baseRef: fullRef, remote, branch };
  },
});

...
const typographyProps = defineProperties({
  properties: {
    // Font size uses raw CSS vars from theme.base.css :root (static, not per-theme)
    fontSize: {
    ...
    // Font weight refs from :root (static)
    fontWeight: {
...
// Radius uses raw CSS vars from theme.base.css :root (static, not per-theme)
const RADIUS = {
  '0': '0',
  xs: 'var(--radius-xs)',
  sm: 'var(--radius-sm)',

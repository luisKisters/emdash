 ...
 *
 * Mirrors HeightModel's responsibilities but returns full geometry
 * (positions + heights) rather than just heights.
 *
...
export class LayoutStore {
  ...
  private _thinkingLayout(item: ChatThinking, viewState: ViewStateStore): MessageLayout {
    ...
    if (item.status === 'thinking') {
      ...
      // The engine seeds setCollapsed(id, true) on the active→done transition so
      // viewState.isCollapsed returns true unless the user explicitly expanded.

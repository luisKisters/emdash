/**
 * Projected / Imperative Layout Engine — Storybook stories.
 *
 * Uses the fully imperative renderer (no React in the hot path).
 * Open these alongside Chat/ChatTranscript stories to compare profiles.
 */
...
`.trim(),
  },
  {
    ...
    name: 'read_file',
    status: 'done',
    inputSummary: 'packages/ui/src/chat/projected/layout/layout-prose.ts',
  },
  {
    kind: 'message',
    id: 'msg-3',
...
function TranscriptWrapper({ stickToBottom }: StoryArgs) {
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column' }}>
      <ProjectedTranscript store={store.current} stickToBottom={stickToBottom} />
    </div>
  );
}
...
function StreamingWrapper() {
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column' }}>
      <ProjectedTranscript store={store} stickToBottom />
    </div>
  );
}
...
// ── Large transcript (compare with Chat/ChatTranscript/LargeTranscript) ────────
...
function LargeTranscriptWrapper() {
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column' }}>
      <ProjectedTranscript store={store.current} stickToBottom={false} />
    </div>
  );
}
...
// Proves the new slot contract: renderCode returns a plain DOM node.
...
function SlotDemoWrapper() {
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column' }}>
      <ProjectedTranscript store={store.current} slots={slots} stickToBottom={false} />
    </div>
  );
}
...
export const Empty: Story = {
  render: () => (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column' }}>
      <ProjectedTranscript store={new TranscriptStore()} />
    </div>
  ),
};

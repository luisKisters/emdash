 ...
 *
 * The React wrapper (view/chat-transcript.tsx) constructs this engine once on
 * mount and calls dispose() on unmount. No per-frame React reconciliation.
 *
 * ## Streaming text updates
 *
 * Streaming ChatMessage rows use a two-channel update model:
 *
 * 1. **Structural channel** (autorun): fires when `store.items` changes (new
 *    rows appended). Mounts new rows; skips already-mounted rows.
 *
 * 2. **Per-row patch channel** (reaction on `item.text`): fires on every
 *    streaming chunk for a mounted streaming row. Calls `patchMessageContent`
 *    which rebuilds only the content div in place (bubble stays mounted, no
 *    positional flash). New lines receive the `.pline--reveal` fade animation.
 *
 * 3. **Streaming→done transition**: when `finalizeTurn` commits the turn,
 *    `_renderVisible` detects `row.wasStreaming && !currentItem.streaming` and
 *    forces a clean remount without the streaming cursor.
 */
...
import { patchMessageContent, renderMessage } from '../dom/render-message';
...
export class ImperativeChat {
  ...
  private readonly slots: ChatSlots | undefined;
  ...
  private _renderVisible(): void {
    ...
    for (let i = start; i <= end; i++) {
      if (this.pool.has(i)) {
        ...
        // Streaming→done transition: if this row was mounted while the message
        // was streaming but the committed item is now finalised (!streaming),
        // force a clean remount so the cursor is removed and content is final.
        if (
          row.wasStreaming &&
          currentItem?.kind === 'message' &&
          !(currentItem as ChatMessage).streaming
        ) {
          this.pool.unmount(i);
          // Fall through to _mountRow below.
        } else {
  ...
  private _mountRow(i: number, item: ChatItem): void {
    ...
    if (item.kind === 'message') {
      ...
      if (item.streaming && result.patchRefs) {
        wasStreaming = true;
        const { bubbleEl, contentEl } = result.patchRefs;

        let islandDisposers: Array<() => void> = [];

        const onIslandChange = (blockId: string, height: number) => {
          this.layoutStore.setMeasured(blockId, height);
          onHeightChange();
        };

        // Per-row reaction: fires on each streaming chunk.
        // Patches only the content div — bubble stays mounted, no flicker.
        const reactionDispose = reaction(
          () => (item as ChatMessage).text,
          () => {
            // Invalidate stale cache entry (key includes text-independent fields;
            // text itself is not part of the key, so cache would be stale).
            this.layoutStore.invalidateItem(item.id);

            const patchResult = patchMessageContent(
              bubbleEl,
              contentEl,
              item as ChatMessage,
              this.layoutStore,
            ...
            islandDisposers = patchResult.disposers;

            // Update virtualizer height + canvas.
            const delta = this.virt.setSize(i, patchResult.newHeight);
            this.canvasEl.style.height = `${this.virt.total()}px`;

            // Scroll-anchor correction if row is above viewport.
            if (delta !== 0) {
              const rowTop = this.virt.top(i);
              if (rowTop < this.scrollEl.scrollTop) {
                this.scrollEl.scrollTop += delta;
        ...
        dispose = () => {
          reactionDispose();
          for (const d of islandDisposers) d();
          result.dispose();
        };
      } else {
    ...
    this.pool.register(i, { node: container, dispose, wasStreaming });
  }

 ...
 *   DIFF_HEADER_H + preview.length * codeLineHeight + 2 * DIFF_PAD_Y
 *
...
import { DIFF_CONTEXT, DIFF_HEADER_H, DIFF_MAX_LINES, DIFF_PAD_Y } from './metrics';
...
export function estimateDiff(fonts: FontConfig): number {
  return DIFF_HEADER_H + DIFF_MAX_LINES * fonts.code.lineHeight + 2 * DIFF_PAD_Y;
}
...
export function measureDiff(item: ChatDiff, fonts: FontConfig): DiffMeasureResult {
  ...
  const height =
    previewRows.length === 0
      ? DIFF_HEADER_H
      : DIFF_HEADER_H + previewRows.length * fonts.code.lineHeight + 2 * DIFF_PAD_Y;

  return { height, previewRows, adds, dels, lang };
}

...
import { VERIFIER_CLASSES, type SelectedVerifier } from './verifier-catalog';
...
const selectedVerifierSchema: z.ZodType<SelectedVerifier> = z.discriminatedUnion('kind', [
  z
    .object({
      kind: z.literal('detected'),
      id: z.string(),
      class: z.enum(VERIFIER_CLASSES),
      label: z.string(),
      command: z.string(),
    })
    .strict(),
...
const loopVerifierPlanSchema = z.array(selectedVerifierSchema).max(64);
